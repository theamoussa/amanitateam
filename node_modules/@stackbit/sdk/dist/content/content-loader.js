...
async function loadContent({ dirPath, config, skipUnmodeledContent }) {
    ...
    const validationResult = content_validator_1.validateContentItems({ contentItems, config });
...
async function readDirRecursivelyWithFilter(dirPath, excludedFiles, allowedExtensions) {
    return utils_1.readDirRecursively(dirPath, {
...
async function loadContentItems({ projectDir, contentDir, filePaths, models, config, objectTypeKeyPath, modelTypeKeyPath, skipUnmodeledContent }) {
    ...
    const contentItems = [];
    const errors = [];
    await utils_1.forEachPromise(filePaths, async (filePath) => {
        ...
        const matchedModels = utils_2.getModelsByQuery({
            filePath: filePath,
...
async function loadFile(filePath, fileIsInProjectDir = false) {
    let data = await utils_1.parseFile(filePath);
    const extension = path_1.default.extname(filePath).substring(1);
...
function modeledDataItem(filePath, data, model, config) {
    ...
    if (utils_2.isListDataModel(model) && lodash_1.default.isArray(data)) {
        data = { items: data };
    }
    return {
        __metadata: {
...
function addMetadataRecursively({ value, model, modelsByName, pageLayoutKey, objectTypeKey, valueId }) {
    ...
    function _mapDeep({ value, model, field, fieldListItem, valueKeyPath, modelKeyPath }) {
        let modelField = null;
        if (field && utils_2.isModelField(field)) {
            modelField = field;
        }
        else if (fieldListItem && utils_2.isModelListItems(fieldListItem)) {
            modelField = fieldListItem;
        }
        if (lodash_1.default.isPlainObject(value) && modelField) {
            const modelResult = utils_2.getModelOfObject({
                object: value,
                field: modelField,
                modelsByName,
                pageLayoutKey,
        ...
        if (lodash_1.default.isPlainObject(value)) {
            ...
            let fieldListItems;
            if (field && utils_2.isListField(field)) {
                fieldListItems = utils_2.getListFieldItems(field);
            }
            else if (model && utils_2.isListDataModel(model)) {
...
function getFirstExistingFile(fileNames, inputDir) {
    return utils_1.findPromise(fileNames, (fileName) => {
