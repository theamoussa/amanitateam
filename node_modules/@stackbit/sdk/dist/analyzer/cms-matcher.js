...
async function matchCMS(options) {
    ...
    let cssMatch = null;
    await utils_1.forEachPromise(CSSMatchers, async (cssMatcher) => {
        if (cssMatcher.match) {
            ...
            const dirs = await analyzer_utils_1.findDirsWithPackageDependency(fileBrowser, lodash_1.default.castArray(cssMatcher.matchByPackageName));
            if (dirs.length === 1) {
                cssMatch = {
                    cmsName: cssMatcher.name
                };
...
const CSSMatchers = [
    ...
    },
    {
        ...
        match: async (fileBrowser) => {
            ...
            const rootConfigPath = await utils_1.findPromise(configFilePaths, async (configFilePath) => {
    ...
    },
    {
        ...
        match: async (fileBrowser) => {
            ...
            const configFilePaths = fileBrowser.getFilePathsForFileName(configFile);
            const netlifyCMSConfigFilePaths = await utils_1.reducePromise(configFilePaths, async (filePaths, filePath) => {
