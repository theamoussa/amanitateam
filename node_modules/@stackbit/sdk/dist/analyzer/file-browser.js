...
class FileSystemFileBrowserAdapter {
    ...
    async listFiles({ includePattern, excludePattern }) {
        const readDirResult = await utils_1.readDirRecursively(this.dirPath, {
            includeDirs: true,
            includeStats: true,
            filter: (filePath) => {
...
class GitHubFileBrowserAdapter {
    ...
    async listFilesRecursively(treeSha, listFilesOptions, parentPath) {
        ...
        const folderResults = await utils_1.reducePromise(filteredFolders, async (accum, treeNode) => {
...
class FileBrowser {
    ...
    findFiles(pattern) {
        return micromatch_1.default(this.filePaths, pattern);
    }
